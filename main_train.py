...
def parse_option():
    ...
    parser.add_argument('--batch_size', type=int, default=64,
                        help='batch_size')
    parser.add_argument('--classifier_batch_size', type=int, default=288,
                        help='batch_size')
...
def set_loader(opt):
    ...
    if opt.dataset == 'path':
        train_dataset1 = LoadData("./NWPU_train_imb_0.01.txt", TwoCropTransforms(train_transform),True)
        train_dataset2 = LoadData("./NWPU_train_imb_0.01.txt", TwoCropTransform(transform_train), True)
        test_dataset = LoadData("./NWPU_test_imb_0.01.txt", TwoCropTransform(transform_test), True)
        self_con_dataset = LoadData("./self_con.txt", THRCropTransforms(train_transform), True)
    else:
    ...
    self_con_loader = torch.utils.data.DataLoader(
        self_con_dataset, batch_size=48, shuffle=(test_sampler is None),
        num_workers=opt.num_workers, pin_memory=True, sampler=test_sampler)
